...
export const SearchForm = ({ searchSubmit, keyWord: keyWordDefault, checked, onCheck }) => {
    ...
    return (
        <form className="searchform" onSubmit={onSubmit}>
            ...
            <input className="searchform__form-input" type="text" placeholder="Фильм" required value={keyWord} onChange={ev => setKeyWord(ev.target.value)} />
            <button className="searchform__form-btn" type="submit" disabled={keyWord.length === 0}>
            ...
            <div className="searchform__form-filter">
                <label className="searchform__form-checkbox">
                    <input type="checkbox" checked={searchCheck} onChange={onCheckHandler} />
                    <span className="slider round"></span>
                </label>
                <div></div>
                <div className="searchform__form-text">Короткометражки</div>
            </div>
            {keyWord.length === 0 && <span className="searchform__form-mes">Нужно ввести ключевое слово</span>}
        </form>
    )
}